'use client';
import classNames from 'classnames';

export const HelpIcon = ({ isActive }: { isActive: boolean }) => (
  <svg
    width="24"
    height="24"
    className={classNames('transition-all duration-300', {
      'stroke-alpina-orange fill-alpina-orange': isActive,
      'stroke-white fill-white': !isActive,
    })}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      opacity="0.4"
      d="M10.49 2.23L5.5 4.10001C4.35 4.53001 3.41 5.89 3.41 7.12V14.55C3.41 15.73 4.19001 17.28 5.14001 17.99L9.44 21.2C10.85 22.26 13.17 22.26 14.58 21.2L18.88 17.99C19.83 17.28 20.61 15.73 20.61 14.55V7.12C20.61 5.89 19.67 4.53001 18.52 4.10001L13.53 2.23C12.68 1.92 11.32 1.92 10.49 2.23Z"
    />
    <path d="M11.9 11.3C11.92 11.3 11.94 11.3 11.96 11.3H11.98H12C13.15 11.26 14.05 10.33 14.06 9.17003C14.06 7.99003 13.1 7.03003 11.92 7.03003C10.74 7.03003 9.78 7.99003 9.78 9.17003C9.8 10.32 10.7 11.26 11.9 11.3Z" />
    <path d="M14.19 13.4101C12.98 12.6101 11.02 12.6101 9.79999 13.4101C9.23999 13.7901 8.91 14.3301 8.91 14.8901C8.91 15.4601 9.22999 15.9801 9.79999 16.3601C10.41 16.7601 11.2 16.9701 11.99 16.9701C12.78 16.9701 13.58 16.7701 14.18 16.3601C14.74 15.9801 15.07 15.4401 15.07 14.8801C15.08 14.3101 14.76 13.7901 14.19 13.4101Z" />
  </svg>
);
